...
// Pure standings types + resolution logic. Kept supabase-free (separate from
// standingsQueries.ts, which imports the RN supabase client) so the seeding
// logic can be unit-tested in a plain node environment. standingsQueries
// re-exports these, so consumers still import from '@/utils/league/standingsQueries'.
