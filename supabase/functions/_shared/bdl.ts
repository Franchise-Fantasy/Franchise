 ...
 *
 * The logic is pure and lives in utils/sports/bdlDates.ts so the client test
 * runner can cover it (same treatment as mapGameStatus above). Re-exported
 * here so edge call sites keep importing from `_shared/bdl.ts`.
 */
export { bdlGameSlateDate } from '../../../utils/sports/bdlDates.ts';
