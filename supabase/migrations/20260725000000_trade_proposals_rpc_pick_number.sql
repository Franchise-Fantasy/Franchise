-- Surface each traded pick's OVERALL pick number (`draft_picks.pick_number`)
-- in the trade-proposals payload so the trade detail / history view can render
-- "2026 2nd · Pick 13" once the draft order is set — matching the trade BUILDER,
-- which already shows the number (via tradablePicks' display_pick). Before this,
-- the deciding manager only saw the round ("2026 2nd") even when the concrete
-- pick was known, because the RPC never selected dp.pick_number.
--
-- pick_number is null until the order is determined (lottery / advance-season /
-- import), which is exactly when formatPickLabel should omit the number rather
-- than invent one — so we pass the raw column through and let the client degrade.
--
-- Pure additive change: adds one field (pick_number) to each item object.
-- Rebased on 20260705000000_trade_proposals_rpc_pick_conditions.sql.
--
-- Also restores `is_in_draft` to the top-level payload. That pick-conditions
-- rewrite accidentally dropped it (added by 20260530180000), leaving the client
-- field always undefined — so useTradeDetailActions read it as falsy and gave
-- mid-draft trades the full veto/review window instead of executing them
-- immediately on accept. Re-select the column so draft trades bypass review as
-- designed.
...
BEGIN
  ...
  enriched_items AS (
    SELECT
      ...
      -- Overall pick number; null until the draft order is set (lottery /
      -- advance-season / import). Lets the client show "· Pick N" when known.
      dp.pick_number AS pick_number,
      ot.name     AS pick_original_team_name,
      -- The pick's CURRENT protection (from any prior trade), distinct from the
      -- protection THIS trade adds (protection_threshold above).
      dp.protection_threshold AS pick_protection_threshold,
      po.name                 AS pick_protection_owner_name
    ...
    LEFT JOIN teams       po ON po.id = dp.protection_owner_id
  ),
  proposal_items AS (
    SELECT
      ei.proposal_id,
      jsonb_agg(jsonb_build_object(
        ...
        'pick_number',              ei.pick_number,
        'pick_original_team_name',  ei.pick_original_team_name,
        'protection_threshold',     ei.protection_threshold,
        'pick_protection_threshold',   ei.pick_protection_threshold,
        'pick_protection_owner_name',  ei.pick_protection_owner_name,
