-- Fix trade messages showing UUID and team attribution in conversation preview.
-- Trades (and rumors) are league-level announcements, not attributable to a team.
...
RETURNS TABLE (
  ...
  latest_msgs AS (
    SELECT DISTINCT ON (msg.conversation_id)
      ...
      END AS content,
      msg.created_at,
      -- Trades and rumors are league-level announcements; hide team attribution
      CASE
