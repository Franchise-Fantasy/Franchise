-- Extend the shared blocked-content moderation to commissioner_announcements.
--
-- The check_blocked_content() trigger function previously covered chat_messages,
-- teams, leagues, commissioner_polls, commissioner_surveys, and survey_questions
-- but had no branch for commissioner_announcements — and no trigger was attached
-- to that table. Commissioners could insert announcement text with prohibited
-- language directly via the client, bypassing the same filter the chat input
-- enforces. This fixes both: the function gains an announcements branch (same
-- shape as the others — `content` column), and a BEFORE INSERT/UPDATE trigger
-- is attached.
...
DROP TRIGGER IF EXISTS trg_moderate_announcements ON public.commissioner_announcements;
CREATE TRIGGER trg_moderate_announcements
  BEFORE INSERT OR UPDATE ON public.commissioner_announcements
  FOR EACH ROW
  EXECUTE FUNCTION public.check_blocked_content();
