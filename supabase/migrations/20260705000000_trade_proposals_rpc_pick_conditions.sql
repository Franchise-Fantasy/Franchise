-- Surface a pick's CURRENT protection in the trade-proposals payload so the
-- trade detail view can show that a pick already carries a condition — distinct
-- from a protection THIS trade is adding (trade_proposal_items.protection_threshold).
-- Before this, a pending trade including an already-protected pick rendered it as
-- an unconditioned asset, hiding the reversion right from the deciding manager.
-- Adds two item fields: pick_protection_threshold + pick_protection_owner_name.
