-- Free-agency roster guard (v3): exclude TAXI from the active-roster count.
--
-- The canonical model (see utils/roster/overCap.ts) is:
--   active roster = roster_slot NOT IN ('IR', 'TAXI')
--   leagues.roster_size = capacity of that active pool; IR and TAXI are extra.
--
-- v2 only excluded 'IR' from v_active_count (and from the pending-drop join),
-- so a player parked on the taxi squad inflated the active count and tripped a
-- false "roster_full" error when adding a free agent — even though the
-- client-side preflight (overCap.ts) correctly excludes TAXI. v3 aligns the
-- RPC with the client by excluding both 'IR' and 'TAXI'.
