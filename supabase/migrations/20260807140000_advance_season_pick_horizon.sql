-- Let `p_new_picks` carry its own season so one rollover can insert picks for
-- more than just the upcoming rookie draft.
--
-- `leagues.max_future_seasons` means "N rookie drafts ahead of the league's
-- current season are tradable" — the inclusive start-year range
-- [season, season + N] that `execute-trade` validates pick swaps against and
-- that the draft hub / trade builder / roster page render. It was seeded ONCE
-- at league creation (`buildFutureDraftPicks`, offsets 1..N) and nothing ever
-- extended it: every rollover moved `leagues.season` forward while the picks
-- stayed put, so the horizon shrank a year per season until a long-running
-- dynasty had no future picks left to trade. advance-season now also seeds the
-- newly-in-range far season(s), which need an explicit season on each row.
--
-- Signature is unchanged (a new parameter would create an overload and make
-- the PostgREST call ambiguous), and `COALESCE(..., p_new_season)` keeps a
-- payload from an older deployed edge function working unchanged.
...
CREATE OR REPLACE FUNCTION public.advance_season_atomic(
  ...
  p_new_season text,
  p_new_picks jsonb,           -- dynasty: picks to INSERT, each with its own
                               -- 'season' (defaults to p_new_season)  (may be [])
...
BEGIN
  ...
  -- 6. Dynasty: seed next season's rookie picks, extend the tradable horizon by
  --    the season that just came into range, or renumber the existing ones.
  ...
  IF jsonb_array_length(COALESCE(p_new_picks, '[]'::jsonb)) > 0 THEN
    ...
    SELECT p_league_id, COALESCE(p->>'season', p_new_season), (p->>'round')::integer,
           (p->>'slot_number')::integer, (p->>'pick_number')::integer,
           (p->>'current_team_id')::uuid, (p->>'original_team_id')::uuid
...
-- Re-asserted because CREATE OR REPLACE keeps existing grants but a future
-- drop/recreate would not; `REVOKE ... FROM PUBLIC` is what actually closes
-- anon, since the implicit PUBLIC grant survives revoking the named roles.
